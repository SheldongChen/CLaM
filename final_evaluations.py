...
def calc_cosine_topk(text_embeddings, motion_embeddings):

    dist_mat = cosine_distance_matrix(text_embeddings.cpu(),
                                                     motion_embeddings.cpu())  
    ...
    for motion_loader_name, motion_loader in motion_loaders.items():
        ...
        with torch.no_grad():
            for idx, batch in enumerate(motion_loader):
                ...
                top_k_mat_cosine = calc_cosine_topk(text_embeddings, motion_embeddings)
                top_k_count_cosine += top_k_mat_cosine.sum(axis=0)


                dist_mat = euclidean_distance_matrix(text_embeddings.cpu().numpy(),
                                                     motion_embeddings.cpu().numpy())
                pos_distance = dist_mat.trace()/32
                neg_distance = (dist_mat.sum()-dist_mat.trace())/(32*31)
#                 print(pos_distance,neg_distance, neg_distance/pos_distance )
...
#                 print(top_k_mat)
#                 print(caption)
                list_save.append((top_k_mat, caption, argsmax))
            ...
            R_precision_cosine = top_k_count_cosine / all_size
            match_score_dict[motion_loader_name] = matching_score
            R_precision_dict[motion_loader_name] = R_precision
            if opt_main.use_topk_cosine:
                print('****use cosine R_precision as R_precision in final statistics!***')
                R_precision_dict[motion_loader_name] = R_precision_cosine
            activation_dict[motion_loader_name] = all_motion_embeddings
            

#         save_variable(list_save, 'list_save_bad.save')                
        ...
        line = f'---> [{motion_loader_name}] R_precision_cosine: '
        for i in range(len(R_precision_cosine)):
            line += '(top %d): %.4f ' % (i+1, R_precision_cosine[i])
        print(line)
        print(line, file=file, flush=True)
...
if __name__ == '__main__':
    ...
    parser_main.add_argument('--gpu', type=int, default=0)
    parser_main.add_argument('--only_gt', action='store_true', )
    parser_main.add_argument('--use_topk_cosine', action='store_true', )
    opt_main = parser_main.parse_args()
    print(opt_main)
    ...
    device_id = int(opt_main.gpu)
    assert device_id in [0,1,2,3]
    ...
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(0)  # 为所有的GPU设置种子  
    
    
    gt_loader, gt_dataset = get_dataset_motion_loader(dataset_opt_path, batch_size, device)
    wrapper_opt = get_opt(dataset_opt_path, device)
    wrapper_opt.use_transformers = True
    print(wrapper_opt)
    eval_wrapper = EvaluatorModelWrapper(wrapper_opt)

    log_file = './t2m_evaluation_chatgpt.log'
    evaluation(log_file)
