...
def build_models(opt):
    ...
    if not opt.use_transformers:
        ...
        if opt.use_transformers_pretrain:
            print("load pretain model of transformer in ./pretrain_clip/ViT-B-32.pt")
            model_pretrain = torch.jit.load('./pretrain_clip/ViT-B-32.pt', map_location='cpu')
            text_enc.load_state_dict(model_pretrain.state_dict(),strict=False)
            motion_enc.load_state_dict(model_pretrain.visual.state_dict(),strict=False)
...
if __name__ == '__main__':
    ...
    if opt.use_transformers:
        opt.save_root = pjoin(opt.checkpoints_dir, opt.dataset_name, opt.name+'_infoNCE_tau_'+str(opt.tau)+'_alpha_'+str(opt.alpha)+'_clip_pretrain_chatgpt_lr_'+str(opt.lr)+'_beta2_'+str(opt.beta2)+'_wd_'+str(opt.adamw_weight_decay)+'_eps_'+str(opt.eps))
    else:
        opt.save_root = pjoin(opt.checkpoints_dir, opt.dataset_name, opt.name+'_infoNCE_tau_'+str(opt.tau)+'_alpha_'+str(opt.alpha)+'_gru_chatgpt_lr_'+str(opt.lr))
