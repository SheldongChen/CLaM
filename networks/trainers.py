...
class TextMotionMatchTrainer(object):
    ...
    def __init__(self, args, text_encoder, motion_encoder, movement_encoder):
        ...
        print('set tau as {}, alpha as {}'.format(self.tau, self.alpha))
    ...
    def train_mode(self):
        ...
        if self.opt.movement_encoder_train:
#             print('set movement encoder train')
    ...
    def forward(self, batch_data):
        ...
        if not self.opt.use_transformers:
            ...
            self.text_embedding = self.text_encoder(caption)
        self.text_embedding = self.text_embedding.clone()[self.align_idx]


    def calc_infoNCE(self, tau):

        # [batch_size, embedding_size]
        x = self.text_embedding
        y = self.motion_embedding

        batch_size, _ = self.text_embedding.shape
        ...
        # 计算余弦相似度矩阵
        cosine_similarity_matrix = x_norm @ y_norm.transpose(0, 1) / tau

        return -F.log_softmax(cosine_similarity_matrix, dim=0).trace()/batch_size - F.log_softmax(cosine_similarity_matrix, dim=-1).trace()/batch_size
    ...
    def backward(self):
        ...
        self.loss_infoNCE = self.calc_infoNCE(tau=self.tau)

        if True:
            # alpha = self.alpha
            self.loss = self.alpha * self.loss + (1 - self.alpha) * self.loss_infoNCE

        loss_logs = OrderedDict({})
        loss_logs['loss'] = self.loss.item()
        loss_logs['loss_pos'] = self.loss_pos.item()
        loss_logs['loss_neg'] = self.loss_neg.item()
        loss_logs['loss_infoNCE'] = self.loss_infoNCE.item()
    ...
    def update(self):
        if self.opt.movement_encoder_train:
            self.zero_grad([self.opt_motion_encoder, self.opt_text_encoder, self.opt_movement_encoder])
            loss_logs = self.backward()
            self.loss.backward()
            self.clip_norm([self.text_encoder, self.motion_encoder, self.movement_encoder])
            self.step([self.opt_text_encoder, self.opt_motion_encoder, self.opt_movement_encoder])        
        else:
    ...
    def get_AdamW_optimizer(self, model):
        ...
        optimizer = optim.AdamW(
            [
                ...
                {"params": rest_params, "weight_decay": self.opt.adamw_weight_decay},
            ],
            lr=self.opt.lr,
    ...
    def train(self, train_dataloader, val_dataloader):
        ...
        if self.opt.use_adamw:
            print('use adamw optim!!')
            self.opt_motion_encoder = self.get_AdamW_optimizer(model=self.motion_encoder)
            self.opt_text_encoder = self.get_AdamW_optimizer(model=self.text_encoder)
        else:
            self.opt_motion_encoder = optim.Adam(self.motion_encoder.parameters(), 
                                                 lr=self.opt.lr,
                                                 betas=(self.opt.beta1, self.opt.beta2),
                                                 eps=self.opt.eps,)
            self.opt_text_encoder = optim.Adam(self.text_encoder.parameters(),
                                               lr=self.opt.lr,
                                               betas=(self.opt.beta1, self.opt.beta2),
                                               eps=self.opt.eps,)

        if self.opt.movement_encoder_train:
            self.opt_movement_encoder = optim.Adam(self.movement_encoder.parameters(), lr=self.opt.lr)
        ...
        min_val_loss = np.inf
        min_val_infoNCE_loss = np.inf
        min_val_pos_neg_loss = np.inf
        while epoch < self.opt.max_epoch:
            # time0 = time.time()
            for i, batch_data in enumerate(train_dataloader):
                self.train_mode()
            ...
            with torch.no_grad():
                for i, batch_data in enumerate(val_dataloader):
                    ...
                    loss_infoNCE_pair += self.loss_infoNCE.item()
                    val_loss += self.loss.item()

            loss_pos_pair /= len(val_dataloader) + 1
            loss_neg_pair /= len(val_dataloader) + 1
            loss_infoNCE_pair /= len(val_dataloader) + 1
            val_loss /= len(val_dataloader) + 1
            print('Validation Loss: %.5f Pos_Neg Loss: %.5f Positive Loss: %.5f Negative Loss: %.5f infoNCE Loss: %.5f' %
                  (val_loss, loss_neg_pair + loss_pos_pair, loss_pos_pair, loss_neg_pair, loss_infoNCE_pair))

            if val_loss < min_val_loss:
                self.save(pjoin(self.opt.model_dir, 'finest.tar'), epoch, it)
                min_val_loss = val_loss
            
            if  loss_infoNCE_pair < min_val_infoNCE_loss:
                self.save(pjoin(self.opt.model_dir, 'finest_infoNCE.tar'), epoch, it)
                min_val_infoNCE_loss = loss_infoNCE_pair

            if loss_neg_pair+loss_pos_pair < min_val_pos_neg_loss:
                self.save(pjoin(self.opt.model_dir, 'finest_pos_neg.tar'), epoch, it)
                min_val_pos_neg_loss = loss_neg_pair+loss_pos_pair
