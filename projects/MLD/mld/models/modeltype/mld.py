...
#         )
        ...
        text_enc = TextTransformer(embed_dim=512, context_length=77, vocab_size=49408, transformer_width=512, transformer_heads=8, transformer_layers=12, device='cuda')
        motion_enc = MotionTransformer(input_resolution=49, patch_size=1, width=768, layers=12, heads=768//64, output_dim=512, device='cuda')
        ...
        t2m_checkpoint = torch.load(
            os.path.join(cfg.model.t2m_path, dataname,
                         "text_mot_match/model/finest.tar"),  map_location='cpu')
        
        text_enc.load_state_dict(t2m_checkpoint["text_encoder"])
        self.t2m_textencoder = text_enc
        print('*load*'*50)
        self.t2m_moveencoder.load_state_dict(
            t2m_checkpoint["movement_encoder"])
        
        motion_enc.load_state_dict(
            t2m_checkpoint["motion_encoder"])
        self.t2m_motionencoder = motion_enc
    ...
    def t2m_eval(self, batch):
        ...
        m_lens = torch.div(m_lens,
                           ...
                           rounding_mode="floor")
#         print(feats_rst[..., :-4].shape)
#         print(caption)
#         print(m_lens)

        l_bs, l_l, l_dim = feats_rst.shape
#         print(feats_rst.shape)
        feats_rst = torch.cat((feats_rst, torch.zeros((l_bs, 196-l_l, l_dim), device=feats_rst.device, dtype=feats_rst.dtype)),dim=1)
#         print(feats_rst.shape)
        recons_mov = self.t2m_moveencoder(feats_rst[..., :-4]).detach()
#         print(recons_mov.shape)
        recons_emb = self.t2m_motionencoder(recons_mov, m_lens)
    
        
        l_bs, l_l, l_dim = motions.shape
        motions = torch.cat((motions, torch.zeros((l_bs, 196-l_l, l_dim), device=motions.device, dtype=motions.dtype)),dim=1)
  
        motion_mov = self.t2m_moveencoder(motions[..., :-4]).detach()
        motion_emb = self.t2m_motionencoder(motion_mov, m_lens)

        # t2m text encoder
        text_emb = self.t2m_textencoder(caption)[align_idx]
