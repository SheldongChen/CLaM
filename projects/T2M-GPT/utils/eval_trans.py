...
#         print(caption)
        motion = motion.cuda()
        et, em = eval_wrapper.get_co_embeddings(word_embeddings, pos_one_hots, sent_len, motion, m_length, list(caption))
        ...
        et_pred, em_pred = eval_wrapper.get_co_embeddings(word_embeddings, pos_one_hots, sent_len, pred_pose_eval, m_length, list(caption))
...
def add_mask(input_index, nb_code, pkeep=0.5, use_pad=True ):
    mask = torch.bernoulli(pkeep * torch.ones(input_index.shape,
                                device=input_index.device))
    mask = mask.round().to(dtype=torch.int64)
    if use_pad:
        r_indices = torch.randint_like(input_index, nb_code)*0 + (nb_code+1)
    else:
...
def add_pad(input_index, max_len, nb_code):  
    len_input = input_index.shape[1]
    input_end = (nb_code)*torch.ones((1, 1),
                                device=input_index.device, dtype=input_index.dtype)
    input_pad = (nb_code+1)*torch.ones((1, max_len-len_input-1),
                                device=input_index.device, dtype=input_index.dtype)
    a_indices = torch.cat([input_index, input_end, input_pad],dim=1)
    return a_indices

@torch.no_grad()        
def evaluation_transformer(out_dir, val_loader, net, trans, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, clip_model, eval_wrapper, draw = True, save = True, savegif=False, double_if = False, double_t = 1, mask_if = True) : 
        ...
        for batch in val_loader:
            ...
            for k in range(bs):
                try:
                    ...
                    if double_if:
                        for double_k in range(double_t):
                            index_motion_pad = add_pad(index_motion, trans.block_size//2, trans.num_vq)
                            if mask_if:
                                index_motion_mask = add_mask(index_motion_pad, trans.num_vq, pkeep=0.5 )
                            else:
                                index_motion_mask = index_motion_pad
                            index_motion_re = trans.resample(feat_clip_text[k:k+1], False, xs=index_motion_mask)[:,trans.block_size//2:]
                            # if k==0:
                            #     import pdb;pdb.set_trace()
                            
                            if index_motion_re.shape[1]>0:
                                index_motion = index_motion_re
                            else:
                                print('resample failed!')
                                index_motion = index_motion
            ...
            et_pred, em_pred = eval_wrapper.get_co_embeddings(word_embeddings, pos_one_hots, sent_len, pred_pose_eval, pred_len, list(clip_text))
            
            if i == 0:
                pose = pose.cuda().float()
                
                et, em = eval_wrapper.get_co_embeddings(word_embeddings, pos_one_hots, sent_len, pose, m_length, list(clip_text))
                motion_annotation_list.append(em)
                motion_pred_list.append(em_pred)
...
@torch.no_grad()        
def evaluation_transformer_test(out_dir, val_loader, net, trans, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, best_multi, clip_model, eval_wrapper, draw = True, save = True, savegif=False, savenpy=False, mm_test=False) : 
    if mm_test:
        mm_num_repeat = 30
        mm_num_choice = 10    
    else:
        mm_num_repeat = 2 #30
        mm_num_choice = 1 #10
    print("mm_num_repeat is {}, mm_num_choice is {}".format(mm_num_repeat, mm_num_choice))
    ...
    multimodality = calculate_multimodality(motion_multimodality, mm_num_choice)
